...
from PyQt6.QtWidgets import (
    ...
    QSizePolicy, QFrame, QSplitter, QGridLayout, QApplication
)
...
# Import necessary classes from main.py and core modules
...
class CompleteCellAnalysisScreen(QWidget):
    ...
    def init_ui(self):
        ...
        from main import ThemeToggle  # Import from main file
        ...
        # Back button
        from main import ElegantButton  # Import from main file
    ...
    def _run_pipeline_thread(self, input_file, params):
        ...
        try:
            self.processed_frames, self.output_csv = self.pipeline.process_pipeline(input_file, params)
    ...
    def save_results(self):
        ...
        try:
            # Save annotated TIFF
            source_tiff = Path(self.pipeline.temp_dir) / "annotated_output.tiff"
            if source_tiff.exists():
                target_tiff = Path(output_dir) / f"annotated_{Path(self.input_file).stem}.tiff"
                shutil.copy(source_tiff, target_tiff)
            
            # Save CSV data
            source_csv = Path(self.output_csv)
            if source_csv.exists():
                target_csv = Path(output_dir) / f"tracked_cells_{Path(self.input_file).stem}.csv"
    ...
    def on_frame_changed(self, frame_number):
        ...
        # Update metrics display or other components when frame changes
        # This would update the cell metrics displays
        pass
    ...
    def event(self, event):
        ...
        if event.type() == QEvent.Type.User:
            ...
            if hasattr(self, 'output_csv') and self.output_csv:
    ...
    def update_theme(self):
        ...
        # This would update the styles based on dark/light mode
        pass
    ...
    def add_sidebar_controls(self, layout):
        ...
        # Parameters sections with collapsible design
    ...
    def create_collapsible_stats_panel(self):
        ...
        col1_layout.addLayout(stats_current_frame["layout"])  # Fixed: access the layout from the dict
        col1_layout.addLayout(stats_position["layout"])  # Fixed: access the layout from the dict
        ...
        col2_layout.addLayout(stats_velocity["layout"])  # Fixed: access the layout from the dict
        col2_layout.addLayout(stats_acceleration["layout"])  # Fixed: access the layout from the dict
        ...
        col3_layout.addLayout(stats_area["layout"])  # Fixed: access the layout from the dict
        col3_layout.addLayout(stats_total_distance["layout"])  # Fixed: access the layout from the dict
        col3_layout.addLayout(stats_direction["layout"])  # Fixed: access the layout from the dict
    ...
    def setup_metrics_tab(self):
        ...
        # Add statistics data
        self.add_metric_stat(stats_grid, 0, 0, "Frames Tracked:", "120/120")
        self.add_metric_stat(stats_grid, 1, 0, "Track Stability:", "96.7%")
        self.add_metric_stat(stats_grid, 0, 1, "Average Velocity:", "2.8 px/frame")
        self.add_metric_stat(stats_grid, 1, 1, "Max Velocity:", "5.2 px/frame at frame 67")
        self.add_metric_stat(stats_grid, 0, 2, "Total Distance:", "342.6 px")
        self.add_metric_stat(stats_grid, 1, 2, "Net Displacement:", "156.3 px")
    ...
    def setup_analysis_tab(self):
        ...
        # Implementation for analysis tab would go here...
        # Similar to the metrics tab layout
    ...
    # Functions to set up the metrics tab charts
    ...
    # Similarly implement other metrics charts...
